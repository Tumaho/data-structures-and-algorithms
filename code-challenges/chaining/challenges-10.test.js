...
You may want to use filter, map, or reduce for this problem, but are not required to. You may need to use
...
let findShortest = (data) => {
  ...
  let str;
  data.map(val =>{
    arr.push(parseInt(val.height));
  })
    
  let min = Math.min.apply(null, arr);
  data.map(val =>{
    if(min == parseInt(val.height) ){
      str=val.name;
    }
  })
  return str;
