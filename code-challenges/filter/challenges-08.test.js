...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  let result=arr.filter(val =>{
    if(val.baseStat > minBaseStat) return true;
  })
  return result;
};
...
Write a function named getStatName that is an extension of your getBaseStatGreaterThan function from
 challenge 4. For this function, extend your solution from challenge 4 to only return the name of the stat,
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  let result=arr.filter(val =>{
    var ol = Object.keys(val);
    if(ol.length == 3) return true;
