...
const toTitleCase = (arr) => {
  // Solution code here...
  let title= arr.map(string => string.slice(0,1).toUpperCase() + string.slice(1));
    return title;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let bigger = arr.filter(element => element.mass > 77);
  let bigg= bigger.map(element => element.name)
  let big= bigg.join(' - ');
  return big;
};
