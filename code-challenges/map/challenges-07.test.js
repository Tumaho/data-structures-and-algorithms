...
Write a function named forLoopTwoToThe that, given an array of integers as input, iterates over the array
 and returns a new array. The returned array should contain the result of raising 2 to the power of the 
...
Write a function named extractStats that, given an array of stats, uses map to return an array of 
...
const extractStats = (arr) => {
  // Solution code here...
  let newArr = arr.map(val =>{
    let sum = val.effort + val.baseStat;
    let names = val.stat.name;
    return {name:names,total:sum};
  })
  return newArr;
};
